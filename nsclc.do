...
putexcel clear
 ...
 stci, by(therapy_type) rmean
 stcox therapy_type
 sts graph, by(therapy_type) title("Progression-Free Survival for All Patients in Dataset") subtitle("by Therapy, Pre-Matching") xtitle ("Survival Time from Treatment Initiation  (Days)") ytitle("Proportion at Risk") legend(order(1 "First-Line Chemotherapy" 2 "First-Line Combination Therapy")) 
  window manage forward results
...
drop if hispanicrace==1 | othercnsmetastases==1 | adrenalmetastases==1 | digestivesystemmetastases==1
tab therapy_type if pdl1>=0.5
teffects ipwra (progression_outcome) (therapy_type) if pdl1>=0.5 
teffects ipwra (progression_outcome ${indiv_covar_psm_prog_outcome}, logit) (therapy_type ${psm})  if pdl1>=0.5 
tebalance summarize
tebalance summarize, baseline

tab therapy_type if pdl1>=0.01 & pdl1 < 0.50
teffects ipwra (progression_outcome) (therapy_type) if pdl1>=0.01 & pdl1<0.5
teffects ipwra (progression_outcome ${indiv_covar_psm_prog_outcome}) (therapy_type ${psm}) if pdl1>=0.01 & pdl1 < 0.50
tebalance summarize
tebalance summarize, baseline
...
teffects ipwra (progression_outcome ${indiv_covar_psm_prog_outcome}) (therapy_type ${psm}) if pdl1==0.0 & pdl1reported==1
tebalance summarize
tebalance summarize, baseline
...
putexcel clear
 ...
 stci, by(therapy_type) rmean
 stcox therapy_type
 sts graph, by(therapy_type) title("Progression-Free Survival for All Patients in Dataset") subtitle("by Therapy, Pre-Matching") xtitle ("Survival Time from Treatment Initiation  (Days)") ytitle("Proportion at Risk") legend(order(1 "First-Line Combination Therapy" 2 "First-Line IO Monotherapy")) 
  window manage forward results
 graph export "prog_survival_without_mutations_pre_match_no_combo_prog.png", replace
 sts test therapy_type, logrank
...
drop if hispanicrace==1 | othercnsmetastases==1 | adrenalmetastases==1
tab therapy_type if pdl1>=0.5
teffects ipwra (progression_outcome) (therapy_type) if pdl1>=0.5 
teffects ipwra (progression_outcome ${indiv_covar_psm_prog_outcome}, logit) (therapy_type ${psm})  if pdl1>=0.5 
tebalance summarize
