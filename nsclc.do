...
global small_covar "i.ecog0 i.ecog1 i.ecog2 i.ecog3 i.ecog4 i.nonsquamouscellcarcinoma diagnosisyear ageatdiagnosis i.academicmedicalcenter glucocorticoidusepriortotreatmen antiinfectiveusepriortotreatment"



global indiv_covar_psm_prog_outcome "i.braf i.kras i.ecog0 i.ecog1 i.ecog2 i.ecog3 i.ecog4 i.nonsquamouscellcarcinoma  diagnosisyear ageatdiagnosis i.white i.asian i.black i.otherrace i.hispanicrace i.patientassistanceprogram i.othergovernmentalinsurance i.medicare i.selfpay i.medicaid i.commercialhealthplan i.noinsurance  i.stage3 i.stage4  i.academicmedicalcenter i.kidney_bool i.liver_bool i.connective_tissue_bool i.interstitiallungdisease  i.bonemetastases i.brainmetastases  i.digestivesystemmetastases   i.antiinfectiveusepriortotreatment glucocorticoidusepriortotreatmen i.diabetes i.male"
...
teffects psmatch (progression_outcome) (therapy_type $psm_no_pdl1) if pdl1>=0.29, vce(robust) caliper(0.2) 
teffects psmatch (progression_outcome) (therapy_type $psm_no_pdl1) if pdl1>=0.39, vce(robust) caliper(0.2)
teffects psmatch (progression_outcome) (therapy_type $psm_no_pdl1) if pdl1>=0.49, vce(robust) caliper(0.2)
teffects psmatch (progression_outcome) (therapy_type $psm_no_pdl1) if pdl1>=0.59, vce(robust) caliper(0.2)
teffects psmatch (progression_outcome) (therapy_type $psm_no_pdl1) if pdl1>=0.69, vce(robust) caliper(0.2)
...
teffects psmatch (progression_outcome) (therapy_type $psm) if pdl1>=0.19, vce(robust) caliper(0.2)
teffects psmatch (progression_outcome) (therapy_type $psm) if pdl1>=0.29, vce(robust) caliper(0.2) 
teffects psmatch (progression_outcome) (therapy_type $psm) if pdl1>=0.39, vce(robust) caliper(0.2)
teffects psmatch (progression_outcome) (therapy_type $psm) if pdl1>=0.49, vce(robust) caliper(0.2)
teffects psmatch (progression_outcome) (therapy_type $psm) if pdl1>=0.59, vce(robust) caliper(0.2)
teffects psmatch (progression_outcome) (therapy_type $psm) if pdl1>=0.69, vce(robust) caliper(0.2)
...
rdrandinf progression_outcome pdl1, cutoff(0.5) fuzzy(first_line ) kernel(uniform) seed(0)  ci(.05) wl (0.3) wr(0.6)   
rdrandinf mortality_outcome pdl1, cutoff(0.5) fuzzy(first_line ) kernel(uniform) seed(0)  ci(.05) wl (0.3) wr(0.6)   


rdrandinf progression_outcome pdl1, cutoff(0.5) fuzzy(first_line tsls) kernel(uniform) seed(0)  ci(.05) wl (0.3) wr(0.6)  firststage 
rdrandinf mortality_outcome pdl1, cutoff(0.5) fuzzy(first_line tsls) kernel(uniform) seed(0)  ci(.05) wl (0.3) wr(0.6)  firststage 
