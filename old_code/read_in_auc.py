...
folder = 'clf_xgb/'
io_extenders = ['1','0']
exclude_extenders = ['0', '1']
for exclude_extender in exclude_extenders:
    for io_extender in io_extenders:
        test_data = np.array(np.load(folder + 'test_set_365_10001' + io_extender + exclude_extender + '.npy', allow_pickle=True))[:, -5:]
        ...
        if io_extender == '0':
            ...
            headers = ["Diagnosis Year", "Age At Diagnosis", "Birth Year",  "Hispanic Ethnicity", "No Insurance", "Worker's Compensation ",
                       "Self-Pay", "Patient Assistance Program",  "Other Governmental Insurance", "Medicare", "Medicaid", "Commercial Health Plan",
                             "ALK+", "EGFR+", "KRAS+",  "ROS1+", "BRAF+", "PDL1", "PDL1 Reported",  "HER2/ERBB2+", "MET+",
                        "RET+", "NTRK1+", "NTRK2+", "NTRK3+","Days from Advanced Diagnosis to Treatment", "Therapy Year",
                       "Renal Failure", "Chronic Kidney Disease", "General Renal Disease",
                        "Prior Kidney Transplant", "Cirrhosis", "Hepatitis", "Prior Liver Transplant", "Connective Tissue Disease",
                        "Scleroderma", "Lupus", "Rheumatoid Arthritis", "Interstitial Lung Disease", "Diabetes",
                        "Bone Metastases", "Brain Metastases", "Other CNS Metastases", "Digestive System Metastases",
        ...
        all_dataset = pd.read_csv(folder + 'clf_xgb_prog_365_10001' + io_extender + exclude_extender + '_train.csv')
        all_dataset = all_dataset.drop('Unnamed: 0', axis=1)
        test_dataset = pd.read_csv(folder + 'clf_xgb_prog_365_10001' + io_extender + exclude_extender +'_test.csv')
        test_dataset = test_dataset.drop('Unnamed: 0', axis=1)

        for objective in ["prog", "mort"]:
            file = open(folder + 'clf_xgb_' + objective + '_365_10001' + io_extender + exclude_extender + '.pickle', 'rb')

            with open(folder + 'clf_xgb_' + objective + '_365_10001' + io_extender + exclude_extender + '.pickle', 'rb') as f:
                grid_search = pickle.load(f)

            best_estimator = grid_search.best_estimator_
            X_train_df = pd.read_csv(folder + 'clf_xgb_prog_365_10001' + io_extender + exclude_extender + '_train.csv')
            X_test_df = pd.read_csv(folder + 'clf_xgb_prog_365_10001' + io_extender + exclude_extender + '_test.csv')
        ...
        if io_extender == '0' and exclude_extender == '0':
            prog_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_1000100_xgb_prog_0.70_365_1000100.npy')), axis=1)
            mort_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_1000100_xgb_mort_0.75_365_1000100.npy')), axis=1)
        elif io_extender == '0' and exclude_extender == '1':
            prog_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_1000101_xgb_prog_0.70_365_1000101.npy')), axis=1)
            mort_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_1000101_xgb_mort_0.72_365_1000101.npy')), axis=1)
        elif io_extender == '1' and exclude_extender == '0':
            prog_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_1000110_xgb_prog_0.65_365_1000110.npy')), axis=1)
            mort_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_1000110_xgb_mort_0.67_365_1000110.npy')), axis=1)
        else:
            prog_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_1000111_xgb_prog_0.64_365_1000111.npy')), axis=1)
            mort_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_1000111_xgb_mort_0.66_365_1000111.npy')), axis=1)
        ...
        all_test_data.to_csv(folder + 'test_set_' + limit + '_' + extender + '_' + io_extender + exclude_extender +'.csv')
        ...
        whole_dataset = pd.read_csv(folder + 'all_x_static_365_10001' + io_extender + exclude_extender + '.csv').values
        ...
        whole_dataset_pickle = np.array(np.load(folder + 'whole_dataset_pre_impute_365_10001' + io_extender + exclude_extender + '.npy', allow_pickle=True))
        if io_extender == '1':
            whole_dataset_pickle = whole_dataset_pickle[whole_dataset_pickle[:, 35] > 0]
