...
headers_all =  ["Diagnosis Year", "Age At Diagnosis", "Birth Year",  "Hispanic Ethnicity", "No Insurance", "Worker's Compensation ", "Self-Pay", "Patient Assistance Program",  "Other Governmental Insurance", "Medicare", "Medicaid", "Commercial Health Plan",
...
def perform_grid_search(param_grid, clf, X_train, y_train, X_test, y_test, filename_extender, type='rf', weights=None, io_only=0):
    ...
    if io_only == 0:
        headers_long = ["Diagnosis Year", "Age At Diagnosis", "Birth Year",  "Hispanic Ethnicity", "No Insurance", "Worker's Compensation ", "Self-Pay", "Patient Assistance Program",  "Other Governmental Insurance", "Medicare", "Medicaid", "Commercial Health Plan",
                             "ALK+", "EGFR+", "KRAS+",  "ROS1+", "BRAF+", "PDL1", "PDL1 Reported",  "HER2/ERBB2+", "MET+",
                        "RET+", "NTRK1+", "NTRK2+", "NTRK3+",
                             "First-Line Combination Therapy", "First-Line Chemotherapy", "Non-First-Line Chemotherapy",
                       "Anti-ALK Drug", "Anti-EGFR Drug","Anti-BRAF Drug", "Anti-ROS1 Drug", "Anti-RAS Drug",
...
if __name__ == '__main__':
    ...
    for patientID in patientID_to_advanced_diagnosis_date:
        ...
        if patientID in patientID_to_biomarkers:
            for biomarker in ["ALK", "EGFR", "KRAS", "ROS1", "BRAF", "PDL1", "PDL1_given", "HER2/ERBB2", "MET", "RET", "NTRK1", "NTRK2", "NTRK3"]:
    ...
    if use_dx == 0:
        categorical_indices = [3, 4, 6, 15, 16, 17, 18, 19, 33]
    else:
    ...
    for mort_outcome in [0, 1]:
        ...
        if use_dl == 1:
            ...
            stat_dynam_drop = layers.Dropout(0.05)(stat)
            x6 = layers.Dense(1000, activation='relu')(stat_dynam_drop)
            x7 = layers.BatchNormalization()(x6)
            x8 = layers.Dropout(0.05)(x7)
            out = layers.Dense(learning_units, activation='sigmoid')(x8) #, kernel_regularizer=regularizers.L1L2(l1=0.01, l2=0.01))(x8)
            model = keras.Model(inputs= stat, outputs=out)
            early_stopping = callbacks.EarlyStopping(monitor='val_loss', mode='min', patience=10)
            reduce_lr = callbacks.ReduceLROnPlateau(monitor='val_loss', mode='min', factor=0.9, patience=3, min_lr=0.00001)
