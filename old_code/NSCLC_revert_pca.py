...
df = pd.DataFrame(data=(np.load('whole_dataset_365_revert3_sdh_1100011_365_.npy')))
df = df.sort_values(by=[26])
...
headers_all =  ["Diagnosis Year", "Age At Diagnosis", "Birth Year",  "Hispanic Ethnicity", "No Insurance",
                "Worker's Compensation ", "Self-Pay", "Patient Assistance Program", "Other Governmental Insurance",
                "Medicare", "Medicaid", "Commercial Health Plan", "KRAS+",  "ROS1+", "BRAF+", "PDL1", "PDL1 Reported",
                "First-Line Combination Therapy", "First-Line Chemotherapy", "Other First-Line Therapy",
                "Clinical Study Drug Used", "Bevacizumab Used",  "Pembrolizumab Used",
                "Days from Advanced Diagnosis to Treatment", "Therapy Year",  "Renal Failure", "Chronic Kidney Disease",
                "General Renal Disease", "Prior Kidney Transplant", "Cirrhosis", "Hepatitis", "Prior Liver Transplant",
                "Connective Tissue Disease", "Scleroderma", "Lupus", "Rheumatoid Arthritis", "Interstitial Lung Disease",
                "Diabetes", "Bone Metastases", "Brain Metastases", "Other CNS Metastases", "Digestive System Metastases",
                "Adrenal Metastases", "Unspecified Metastases", "SDH", "Glucocorticoid Use Prior to Treatment",
                "Anti-Infective Use Prior to Treatment", "Albumin", "Creatinine", "Bilirubin", "AST", "ALT", "Male",
                "White", "Asian", "Other Race", "Hispanic Race", "Black", "Academic Medical Center", "ECOG 0", "ECOG 1",
                "ECOG 2", "ECOG 3", "ECOG 4", 'Stage 0', 'Stage I',  'Stage II', 'Stage III', 'Stage IV', 'Occult',
                "Squamous Cell Carcinoma", "Nonsquamous Cell Carcinoma", "Never Smoker", "Previous Smoker",
                "First-Line Nivolumab Monotherapy", "First-Line Pembrolizumab Monotherapy",
                "First-Line Cemiplimab Monotherapy", "First-Line Atezolizumab Monotherapy",
                "First-Line Durvalumab Monotherapy", "First-Line Ipilimumab/Nivolumab"]

headers_to_drop = ["First-Line Combination Therapy", "First-Line Chemotherapy",
                       "Other First-Line Therapy", "Clinical Study Drug Used", "Bevacizumab Used", "Pembrolizumab Used",
                        "First-Line Nivolumab Monotherapy",
                       "First-Line Pembrolizumab Monotherapy", "First-Line Cemiplimab Monotherapy",
                       "First-Line Atezolizumab Monotherapy",  "First-Line Durvalumab Monotherapy",
...
if __name__ == '__main__':
    ...
    stages = {'Group stage is not reported': 0, 'Stage 0': 1, 'Stage I': 2, 'Stage IA': 2, 'Stage IA1': 2,
              'Stage IA2': 2, 'Stage IA3': 2, 'Stage IB': 2, 'Stage II': 3, 'Stage IIA': 3, 'Stage IIB': 3,
              'Stage III': 4, 'Stage IIIA': 4, 'Stage IIIB': 4, 'Stage IIIC': 4, 'Stage IV': 5, 'Stage IVA': 5,
              'Stage IVB': 5,    'Occult': 6}
    ...
    for i in range(len(demographics['PatientID'])):
        ...
        patientIDs_demos[patientID] = np.array([birth_year, gender_, race_, ethnicity_])
    ...
    approved_first_line_immunomonotherapies = ["nivolumab", "pembrolizumab", "cemiplimab", "atezolizumab", "durvalumab",
    ...
    for i in range(len(biomarkers['PatientID'])):
        ...
        if patient_ID not in patientID_to_biomarkers:
            patientID_to_biomarkers[patient_ID] = {"KRAS": 0, "ROS1": 0, "BRAF": 0, "PDL1": -1.0,
                                                   "PDL1_given": 0}
    ...
    for patientID in patientID_to_advanced_diagnosis_date:
        ...
        if patientID in patientID_to_biomarkers:
            for biomarker in ["KRAS", "ROS1", "BRAF", "PDL1", "PDL1_given"]:
                all_biomarkers.append(patientID_to_biomarkers[patientID][biomarker])
        else:
            all_biomarkers = [0, 0, 0, 0.0, 0]
        ...
        if use_dx == 0:
            ...
            therapy_info = [io_mono_used, combo_therapy, first_line_chemo, other_first_line_therapy, clinical_study_drug,
                            bevacizumab_used, pembrolizumab_used, days_from_dx_to_tx, therapy_year]
    ...
    file_name_extender = str(min_time) + '_pca_final_'
    ...
    file_name_extender += '_' + str(min_time) + '_'
    ...
    if use_dx == 0:
        categorical_indices = [3, 4, 14, 15, 16, 17, 18, 24]
    else:
        categorical_indices = [3, 4, 14, 15, 16, 17, 18]
    ...
    for mort_outcome in [0, 1]:
        ...
        new_file_name_extender = file_name_extender + '_unweighted'

        perform_grid_search(params_xgb, xgb_model, X_static_train, y_train_final, X_static_test, y_test_final,
                            file_name_extender,  type='xgb' + new_file_name_extender, weights=None, io_only=io_only)
