...
for exclude_extender in exclude_extenders:
    for io_extender in io_extenders:
        test_data = np.array(np.load(folder + 'test_set_365_all_10001' + io_extender + exclude_extender + '.npy', allow_pickle=True))[:, -5:]
        ...
        all_dataset = pd.read_csv(folder + 'clf_' + model_type + '_prog_365_all_10001' + io_extender + exclude_extender +'_train.csv')
        all_dataset = all_dataset.drop('Unnamed: 0', axis=1)
        test_dataset = pd.read_csv(folder + 'clf_' + model_type + '_prog_365_all_10001' + io_extender + exclude_extender + '_test.csv')
        test_dataset = test_dataset.drop('Unnamed: 0', axis=1)

        for objective in ["prog", "mort"]:
            file = open(folder + 'clf_' + model_type + '_' + objective + '_365_all_10001' + io_extender + exclude_extender  + '.pickle', 'rb')

            with open(folder + 'clf_' + model_type + '_' + objective + '_365_all_10001' + io_extender + exclude_extender  + '.pickle', 'rb') as f:
                grid_search = pickle.load(f)

            best_estimator = grid_search.best_estimator_
            X_train_df = pd.read_csv(folder + 'clf_' + model_type + '_' + objective + '_365_all_10001' + io_extender + exclude_extender  + '_train.csv')
            X_test_df = pd.read_csv(folder + 'clf_' + model_type + '_' + objective + '_365_all_10001' + io_extender + exclude_extender  + '_test.csv')
        ...
        if io_extender == '0' and exclude_extender == '1':
            mort_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_all_1000101_xgb_mort_0.718_365_all_1000101.npy')), axis=1)
            prog_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_all_1000101_xgb_prog_0.701_365_all_1000101.npy')), axis=1)
        else:
            prog_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_all_1000111_gb_prog_0.664_365_all_1000111.npy')), axis=1)
            mort_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_365_all_1000111_gb_mort_0.667_365_all_1000111.npy')), axis=1)
        ...
        whole_dataset = pd.read_csv(folder + 'all_x_static_365_all_10001' + io_extender + exclude_extender  + '.csv').values
        ...
        whole_dataset_pickle = np.array(np.load(folder + 'whole_dataset_365_all_10001' + io_extender + exclude_extender + '.npy', allow_pickle=True))

        if io_extender == '1':
            whole_dataset_pickle = whole_dataset_pickle[whole_dataset_pickle[:, 29] > 0]
