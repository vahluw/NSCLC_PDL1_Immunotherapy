...
/*  Chemo therapy vs. first-line IO monotherapy Kaplan-Meier (PDL1 and non-PDL1) */
 ...
 gen interaction_term = pdl1*therapy_type
 stset censor_time, failure(endpoint)
 stci, by(therapy_type) rmean
 stcox therapy_type
...
rdwinselect pdl1 days_from_dx_to_tx therapy_year age_at_diagnosis insured never_smoker prev_smoker black white asian other_race male if practice_type==1, c(0.5) seed(0) reps(1000) level(0.05) wmass
rdrandinf progression_outcome pdl1, cutoff(0.5) fuzzy(first_line tsls) kernel(uniform) seed(0)  ci(.05) wl (0.45) wr(0.56)  firststage 

rdrandinf progression_outcome pdl1, cutoff(0.5) fuzzy(first_line tsls) kernel(uniform) seed(0)  ci(.05) wl (0.149) wr(0.751)  firststage 
rdrandinf progression_outcome pdl1, cutoff(0.5) fuzzy(first_line tsls) kernel(uniform) seed(0)  ci(.05) wl (0.14) wr(.86)  firststage 

rdrandinf progression_outcome pdl1, cutoff(0.5) fuzzy(first_line tsls) kernel(uniform) seed(0)  ci(.05) wl (0.44) wr(0.56)  firststage 

rdrandinf mortality_outcome pdl1, cutoff(0.5) fuzzy(first_line tsls) kernel(uniform) seed(0)  ci(.05) wl (0.0) wr(1) wmass firststage  


drop if therapy_year < 2014 

rdwinselect pdl1 days_from_dx_to_tx therapy_year age_at_diagnosis insured never_smoker prev_smoker black white asian other_race, c(0.5) seed(0) reps(1000) level(0.05) wmass
rdrandinf progression_outcome pdl1, cutoff(0.5) fuzzy(first_line tsls) kernel(uniform) seed(0)  ci(.05) wl (0.149) wr(0.751)  firststage 
rdrandinf mortality_outcome pdl1, cutoff(0.5) fuzzy(first_line tsls) kernel(uniform) seed(0)  ci(.05) wl (0.0) wr(1) wmass firststage  
...
ivreg2 progression_outcome (therapy_type=over_threshold i.practiceid ) $indiv_covar  i.state  therapy_year kidney_failure chronic_kidney_disease renal_disease kidney_transplant cirrhosis hepatitis liver_transplant connective_tissue scleroderma lupus rheumatoid_arthritis  interstitial_lung_disease diabetes bone_mets brain_mets cns_mets digestive_mets adrenal_mets unspecified_mets ast alt creatinine bilirubin // Adjusted regression
