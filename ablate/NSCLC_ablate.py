...
if __name__ == '__main__':
    ...
    file_name_extender = str(min_time) + '_ablation_'
    ...
    for mort_outcome in [0, 1]:
        ...
        if mort_outcome == 0:
            X_ablate_df = X_df.drop(['Stage IV', 'Stage IVA', 'Stage IVB', 'PDL1', 'Medicare', 'Diagnosis Year', 'Albumin', 'ALK+', 'EGFR+'], axis=1)
            X_ablate_df_test = X_df_test.drop(['Stage IV', 'Stage IVA', 'Stage IVB', 'PDL1', 'Medicare', 'Diagnosis Year', 'Albumin', 'ALK+', 'EGFR+'], axis=1)
        else:
            X_ablate_df = X_df.drop(['Stage IV', 'Stage IVA', 'Stage IVB', 'PDL1', 'Medicare', 'PDL1 Reported', 'Albumin', 'ALK+', 'EGFR+'], axis=1)
            X_ablate_df_test = X_df_test.drop(['Stage IV', 'Stage IVA', 'Stage IVB', 'PDL1', 'Medicare', 'Diagnosis Year', 'Albumin', 'ALK+', 'EGFR+'], axis=1)

        X_ablate_df.headers = None
        X_ablate = X_ablate_df.to_numpy(copy=True)
        X_ablate_df_test.headers = None
        X_ablate_df_test_final = X_ablate_df_test.to_numpy(copy=True)
        ...
        perform_grid_search(params_xgb, xgb_model, X_ablate, y_train_final, X_ablate_df_test_final, y_test_final,
        ...
        perform_grid_search(params_gb, gb_clf, X_ablate, y_train_final, X_ablate_df_test_final, y_test_final,
        ...
        perform_grid_search(params_rf, rf_clf, X_ablate, y_train_final, X_ablate_df_test_final, y_test_final,
