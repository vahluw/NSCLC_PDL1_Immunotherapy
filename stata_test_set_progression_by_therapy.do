/*  Chemo therapy vs. first-line IO monotherapy Kaplan-Meier (PDL1 and non-PDL1) */
 ...
 rocreg progression_outcome hgb_preds, bseed(0)
 
 import delimited "/Users/vahluw/Documents/NSCLC_PDL1_Immunotherapy/test_set_182.csv", clear 
 gen threshold = 0.50
 drop if hgb_preds < 0.7 & hgb_preds > 0.3
 ...
 sts graph, by(progressed_prediction) title("Progression-Free Survival for Test-Set Patients") subtitle("by ML-Derived Risk")  xtitle ("Survival Time From Diagnosis Date (Days)") ytitle ("Proportion at Risk") legend(order(1 "Low-Risk" 2 "High-Risk"))
 graph export "io_chemo_test_set_ml_mutations.png", replace
 ...
 stcox progressed_prediction
 sts graph, by(progressed_prediction) title("Progression-Free Survival for Test-Set Patients") subtitle("by ML-Derived Risk")  xtitle ("Survival Time From Diagnosis Date (Days)") ytitle ("Proportion at Risk") legend(order(1 "Low-Risk" 2 "High-Risk"))
 graph export "io_chemo_test_set_ml_no_mutations.png", replace
 ...
 sts graph if therapy_type==0 , by(progressed_prediction) title("Progression-Free Survival for Test-Set Patients on Chemotherapy") subtitle("by ML-Derived Risk")  xtitle ("Survival Time From Diagnosis Date  (Days)") ytitle ("Proportion at Risk") legend(order(1 "Low-Risk" 2 "High-Risk"))
