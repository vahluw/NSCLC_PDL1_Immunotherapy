...
limit = "365"
extender = "10000"
folder = 'clf_xgb_revert3/'
io_extenders = ['1','0']
exclude_extenders = ['1']
final_extender = "_365_"
for exclude_extender in exclude_extenders:
    for io_extender in io_extenders:
        test_data = np.array(np.load(folder + 'test_set_365_revert3_sdh_10001' + io_extender + exclude_extender + '_365_.npy', allow_pickle=True))[:, -5:]
        ...
        all_dataset = pd.read_csv(folder + 'clf_xgb_prog_365_revert3_sdh_10001' + io_extender + exclude_extender + final_extender + '_train.csv')
        all_dataset = all_dataset.drop('Unnamed: 0', axis=1)
        test_dataset = pd.read_csv(folder + 'clf_xgb_prog_365_revert3_sdh_10001' + io_extender + exclude_extender + final_extender + '_test.csv')
        test_dataset = test_dataset.drop('Unnamed: 0', axis=1)

        for objective in ["prog", "mort"]:
            file = open(folder + 'clf_xgb_' + objective + '_365_revert3_sdh_10001' + io_extender + exclude_extender + final_extender + '.pickle', 'rb')

            with open(folder + 'clf_xgb_' + objective + '_365_revert3_sdh_10001' + io_extender + exclude_extender + final_extender + '.pickle', 'rb') as f:
                grid_search = pickle.load(f)

            best_estimator = grid_search.best_estimator_
            X_train_df = pd.read_csv(folder + 'clf_xgb_prog_365_revert3_sdh_10001' + io_extender + exclude_extender + final_extender + '_train.csv')
            X_test_df = pd.read_csv(folder + 'clf_xgb_prog_365_revert3_sdh_10001' + io_extender + exclude_extender + final_extender + '_test.csv')
        ...
        if io_extender == '0' and exclude_extender == '1':
            prog_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_prog_0.71_1000101.npy')), axis=1)
            mort_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_mort_0.72_1000101.npy')), axis=1)
        else:
            prog_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_prog_0.71_1000111.npy')), axis=1)
            mort_gb_preds = np.expand_dims(np.array(np.load(folder + 'y_pred_mort_0.67_1000111.npy')), axis=1)
        ...
        all_test_data.to_csv(folder + 'test_set_' + limit + '_revert3_sdh_' + extender + '_' + io_extender + exclude_extender +'.csv')
        ...
        whole_dataset = pd.read_csv(folder + 'all_x_static_365_revert3_sdh_10001' + io_extender + exclude_extender + final_extender + '.csv').values
        ...
        whole_dataset_pickle = np.array(np.load(folder + 'whole_dataset_365_revert3_sdh_10001' + io_extender + exclude_extender + final_extender + '.npy', allow_pickle=True))
        if io_extender == '1':
