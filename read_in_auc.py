...
all_dataset = np.array(np.load('whole_dataset_0110111.npy'))
test_dataset = np.array(np.load('test_set_0110111.npy'))
...
all_test_data = np.concatenate((test_dataset, gb_preds, xgb_preds, rf_preds, knn_preds, logistic_preds), axis=1)
if new_therapy == 1:
...
headers = ["in_test_set", "contra1", "contra2", "contra3", "contra4", "contra5", "contra6", "bilirubin", "diag_year", "AST", "ALT", "diag_year",
                    "age_at_diagnosis", "birth_year", "gender", "race", "ethnicity", "state", "other_no_insurance",
                    "workers_comp","self_pay","pt_assistance","other_gov_insurance","medicare", "medicaid",
                    "commercial_health_plan", "practice_ID", "practice_type", "physician_ID", "histology",
                "stage", "smoking_status","ecog",  "ALK", "EGFR", "KRAS", "ROS1", "BRAF", "PDL1", "PDL1_given",
